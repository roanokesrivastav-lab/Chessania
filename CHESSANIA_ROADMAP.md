...
1. features.py: PlayerFeatures dataclass (it will grow across S12–S14; define the full shape now with None for not-yet-computed sections, so the contract is visible early). This session computes:
   ...
   * endgame_conversion: of games that reached an endgame while the player was STRICTLY winning (ahead ≥300cp — a decisive edge a sub-1800 should convert ~10/10, incl. a clean extra-pawn endgame), the % actually won. Guard division by zero (no such games → None, not 0 — "no data" and "converts nothing" are very different coaching facts).
   * advantage_capitalization: of games where the player reached a winning advantage (≥300cp from their POV) at ANY point, the % actually won. None when no such games.
   * resourcefulness: of games where the player fell into a tenable-but-losing position (player-POV eval between -600cp and -150cp at its worst), the % that ended as a win or draw. None when no such games.
...
7. Tilt / compounding (added Session 30, Part G #11c) — a mistake or blunder immediately followed by another blunder on the player's own next move. This is the 10th detector in `run_detectors` and surfaces as a behavioral issue rather than a stat tile.
8. Each detector is a pure function over stored move_evals + games — no engine calls (everything needed was stored in S8–S9; if a detector seems to need a new engine call, its design is wrong — raise it, don't call).
9. Calibration: run against the ground-truth games and the founder's account; founder sanity-reads every fired detector's evidence (open the cited moves on the platform and confirm the story is real).
...
class StatsBlock(BaseModel):
    ...
    resourcefulness: float | None          # None = never had a tenable-but-losing position
...
* missed_saves — H: "You collapse positions you could still fight for." D: "You got into a fightable position in {trouble_games} games but only saved {saved} ({rate}%)." P: defensive technique — look for one active defensive resource on every move when down material; study Lichess "defensive tactics" drills. Counter-evidence: counts positions down about a piece or less (-150 to -600) at any point, not only endgames. Success metric: save at least {target}% of such positions over the next 10 games. If comeback rate is high, this surfaces as a Strength instead: "You fought back from a losing position in {comebacks} of {trouble_games} games — that resilience wins points others throw away."
...
* tilt (Session 30, Part G #11c) — H: "Your mistakes are compounding into blunders." D: "In {tilt_games} of your {games_analyzed} games, a mistake or blunder was immediately followed by another blunder — {tilt_events} compounding events in total." P: after any error, treat the next position as brand new — take a slow 10 seconds, reset, and ask what the position needs now. M: "Play 10 games in a row without a compounding blunder." Counter-evidence: counts only back-to-back errors on your own consecutive moves, not scattered blunders.
