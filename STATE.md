...
- **[founder-to-verify] — the S21 DoD**: inspect the `<title>`/metadata on a real `/r/...` share, verify the favicon appears, paste the share URL into a social debugger or chat to confirm the OG card unfurls (or at least that `/r/chesscom/Eleven_14/opengraph-image` returns a 1200×630 PNG), and check that a mistyped URL shows the friendly 404.
